 ...
 * @date 2019/1/6
 */
public class FixedDishLayoutManager2 extends RecyclerView.LayoutManager {

    private final int screenWidth;
    private int itemWidth;
    private int itemHeight;
    private Context context;
    private int rightMargin = dpToPx(20);
    ...
    public FixedDishLayoutManager2(Context context) {
        this(context, 5000);
    }

    public FixedDishLayoutManager2(Context context, int initPosition) {
...
//        int distance = (position-currrentPosition)*halfItemWidth;
    ...
    private void brewAndStartAnimator(int dur, int finalXorY) {
        animator = ObjectAnimator.ofInt(FixedDishLayoutManager2.this, "animateValue", 0, finalXorY);
...
//        detachAndScrapAttachedViews(recycler);
        ...
        oneScreenItemCount = (int) Math.ceil((screenWidth - itemWidth-rightMargin) * 1.0f / halfItemWidth) + 1;
    ...
    public int fill(RecyclerView.Recycler recycler, int dxReverse) {
        ...
        // 暂时将所有view回收，等待处理
        detachAndScrapAttachedViews(recycler);
        for (int j = endPosition; j > startPosition; j--) {
...
//        handleViewByPosition(recycler, startPosition, offset, startPosition).setAlpha(0f);
        handleViewByPosition(recycler, startPosition, offset, currentPosition).setAlpha(calcAlpha(offset));
    ...
    private float calcAlpha(int offset){
        if (offset<=rightMargin){
            return 1;
        }
        if (offset>=halfItemWidth-dpToPx(10)){
            return 0;
        }
        float curAlpha = (halfItemWidth - offset) * 2f / halfItemWidth;
        System.out.println("curAlpha:"+curAlpha);
        return curAlpha;
...
//        System.out.println(">>>>>>>>j=" + j);
        ...
        int left = screenWidth - itemWidth - rightMargin - (j - startPosition) * halfItemWidth + offset;
        int top = 0;
